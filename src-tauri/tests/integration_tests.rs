...
use common::{utils, MockModbusServer};
use modbus_reader::modbus::{AddressRange, AppState, ModbusConfig};
...
async fn test_modbus_read_holding_registers_command() {
    ...
    app_state.set_config(config).await;
    assert!(
        modbus_reader::modbus::manager::modbus_connect(app_state.clone())
            .await
            .is_ok()
    );
    ...
    // 测试读取命令
    let result =
        modbus_reader::modbus::manager::modbus_read_holding_registers(0, 5, app_state.clone())
            .await;
...
async fn test_modbus_read_multiple_ranges_command() {
    ...
    // 准备地址范围
    let ranges = vec![AddressRange::new(0, 2), AddressRange::new(10, 3)];

    // 测试多范围读取命令
    let result =
        modbus_reader::modbus::manager::modbus_read_multiple_ranges(ranges, app_state.clone())
            .await;
...
async fn test_modbus_get_connection_state_command() {
    ...
    // 测试未连接状态
    let state =
        modbus_reader::modbus::manager::modbus_get_connection_state(app_state.clone()).await;
    assert!(matches!(
        state,
        modbus_reader::modbus::ConnectionState::Disconnected
    ));
    ...
    assert!(matches!(
        state,
        modbus_reader::modbus::ConnectionState::Connected
    ));
...
async fn test_modbus_config_commands() {
    ...
    // 测试设置配置命令
    let result =
        modbus_reader::modbus::manager::modbus_set_config(config.clone(), app_state.clone()).await;
...
async fn test_modbus_get_connection_info_command() {
    ...
    // 测试获取连接信息命令
    let result =
        modbus_reader::modbus::manager::modbus_get_connection_info(app_state.clone()).await;
...
async fn test_read_operations_without_connection() {
    ...
    // 未连接时读取寄存器应该失败
    let result =
        modbus_reader::modbus::manager::modbus_read_holding_registers(0, 10, app_state.clone())
            .await;
...
async fn test_error_handling_and_recovery() {
    ...
    assert!(matches!(
        state,
        modbus_reader::modbus::ConnectionState::Error(_)
    ));
}
...
async fn test_command_thread_safety() {
    ...
    for _ in 0..5 {
        ...
        let handle = tokio::spawn(async move {
            ...
            // 并发获取状态操作
            let state =
                modbus_reader::modbus::manager::modbus_get_connection_state(app_state_clone).await;
            assert!(matches!(
                state,
                modbus_reader::modbus::ConnectionState::Connected
            ));
...
async fn test_long_running_stability() {
    ...
    for i in 0..20 {
        ...
        // 读取并验证数据
        let result =
            modbus_reader::modbus::manager::modbus_read_holding_registers(0, 50, app_state.clone())
                .await;
