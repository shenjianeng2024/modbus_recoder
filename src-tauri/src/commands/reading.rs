...
pub async fn read_modbus_ranges(
    ...
    match client
        .read_ranges_detailed(request.ranges, request.format)
        .await
    {
        Ok(result) => {
            log::info!(
...
pub async fn export_csv(file_path: String, data: Vec<BatchReadResult>) -> Result<String, String> {
    ...
    for batch in &data {
        ...
        let mut row = format!(
            "{},{},{},{}",
            batch.timestamp, batch.success_count, batch.failed_count, batch.duration_ms
        );
...
mod tests {
    ...
    fn test_detailed_read_request_serialization() {
        ...
        let request = DetailedReadRequest {
            ranges: vec![AddressRange::new(0, 10), AddressRange::new(100, 5)],
