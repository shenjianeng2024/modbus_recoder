...
mod tests {
    ...
    fn test_float32_byte_order_diagnosis() {
        ...
        let current_result = ModbusClient::create_address_result(
            0, 0x4049, "dec", "2024-01-01T00:00:00Z", None, "float32", Some(0x0FDB)
        );
        ...
        assert!((parsed_float - expected_pi).abs() < 0.0001, 
                "Expected ~{}, got {}", expected_pi, parsed_float);
        
        // 测试反向字节序情况
        let reversed_result = ModbusClient::create_address_result(
            0, 0x0FDB, "dec", "2024-01-01T00:00:00Z", None, "float32", Some(0x4049)
        );
        
        // 这个应该产生不同的结果
        assert_ne!(reversed_result.raw_value, current_result.raw_value);
        println!("Current order (0x4049, 0x0FDB): {} -> {}", 
                 current_result.raw_value, current_result.parsed_value);
        println!("Reversed order (0x0FDB, 0x4049): {} -> {}", 
    ...
    fn test_float32_single_address_fallback() {
        // 测试单地址读取时的fallback行为
        let result = ModbusClient::create_address_result(
            0, 0x4049, "dec", "2024-01-01T00:00:00Z", None, "float32", None
        );
    ...
    fn test_float32_actual_parsing_scenario() {
        ...
        println!("Simulated batch read result: addr={}, raw=0x{:08X}, parsed={}", 
    ...
    fn test_float32_zero_cases() {
        ...
        let result_zero = ModbusClient::create_address_result(
            0, 0x0000, "dec", "2024-01-01T00:00:00Z", None, "float32", Some(0x0000)
        );
        ...
        let result_mixed = ModbusClient::create_address_result(
            0, 0x0000, "dec", "2024-01-01T00:00:00Z", None, "float32", Some(0x4049)
        );
        assert_eq!(result_mixed.raw_value, 0x00004049);
        let parsed_mixed: f32 = result_mixed.parsed_value.parse().unwrap();
        println!("Mixed case (0x0000, 0x4049): raw=0x{:08X}, parsed={}", 
    ...
    fn test_float32_byte_order_support() {
        ...
        let little_endian_result = ModbusClient::create_address_result_with_byte_order(
            0, high_word, "dec", "2024-01-01T00:00:00Z", None, "float32", Some(low_word), &ByteOrder::LittleEndian
        );
        ...
        println!("Big Endian (0x4049, 0x0FDB): raw=0x{:08X}, parsed={}", 
                 big_endian_result.raw_value, big_endian_result.parsed_value);
        println!("Little Endian (0x4049, 0x0FDB): raw=0x{:08X}, parsed={}", 
    ...
    fn test_uint32_int32_byte_order_support() {
        ...
        let uint32_be = ModbusClient::create_address_result_with_byte_order(
            0, high_word, "dec", "2024-01-01T00:00:00Z", None, "uint32", Some(low_word), &ByteOrder::BigEndian
        );
        ...
        let uint32_le = ModbusClient::create_address_result_with_byte_order(
            0, high_word, "dec", "2024-01-01T00:00:00Z", None, "uint32", Some(low_word), &ByteOrder::LittleEndian
        );
        ...
        let int32_be = ModbusClient::create_address_result_with_byte_order(
            0, neg_high, "dec", "2024-01-01T00:00:00Z", None, "int32", Some(neg_low), &ByteOrder::BigEndian
        );
        assert_eq!(int32_be.raw_value, 0xFFFFFFFF);
        assert_eq!(int32_be.parsed_value, "-1");
        
        println!("uint32 BE: 0x{:08X} -> {}", uint32_be.raw_value, uint32_be.parsed_value);
        println!("uint32 LE: 0x{:08X} -> {}", uint32_le.raw_value, uint32_le.parsed_value);
        println!("int32 BE (all FF): 0x{:08X} -> {}", int32_be.raw_value, int32_be.parsed_value);
    }
